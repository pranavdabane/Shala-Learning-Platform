...
const CoursePlayer: React.FC<CoursePlayerProps> = ({ course, currentProgress, onClose, onUpdateProgress }) => {
  ...
  return (
    <div className="fixed inset-0 z-[160] bg-slate-950 flex flex-col animate-in fade-in duration-500 overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 md:px-10 py-4 md:py-6 border-b border-white/5 bg-surface-dark/50 backdrop-blur-xl gap-6">
        <div className="flex items-center gap-4 md:gap-6 text-left w-full sm:w-auto">
          <button onClick={onClose} className="size-10 md:size-12 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all text-white shrink-0 border border-white/10">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Lesson {activeLesson.id}</span>
            </div>
          </div>
        </div>

        <div className="flex items-center justify-between sm:justify-end gap-4 md:gap-8 w-full sm:w-auto">
          <div className="flex items-center gap-3 md:gap-4 bg-white/5 px-4 md:px-6 py-2 md:py-3 rounded-2xl border border-white/10 shadow-inner">
            <span className={`material-symbols-outlined text-sm md:text-base ${isTimerRunning ? 'text-primary animate-pulse' : 'text-slate-500'}`}>timer</span>
          ...
          <button 
            ...
            className="px-6 md:px-10 py-3 md:py-4 bg-primary text-white text-xs md:text-sm font-black rounded-2xl hover:scale-105 transition-all shadow-2xl shadow-primary/20 disabled:opacity-50 flex items-center gap-2 md:gap-3 uppercase tracking-widest"
          >
          ...
          <div className="w-full h-auto lg:h-full aspect-video lg:aspect-auto">
            {course.videoUrl ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center space-y-6 bg-slate-900">
                <div className="size-24 md:size-32 rounded-full bg-slate-800 flex items-center justify-center text-slate-700 animate-pulse">
                  <span className="material-symbols-outlined text-6xl md:text-8xl">play_circle_filled</span>
                </div>
                <p className="text-slate-500 font-bold text-sm md:text-lg uppercase tracking-widest">Video stream unavailable.</p>
              </div>
            )}
          </div>
          
          <div className="w-full lg:absolute lg:top-12 lg:right-12 lg:w-80 p-6 md:p-8 bg-slate-950/60 lg:backdrop-blur-2xl lg:rounded-[32px] border-y lg:border border-white/10 space-y-4 md:space-y-6 transition-all text-left z-20 shadow-2xl">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="size-8 md:size-10 rounded-xl bg-primary/20 text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-all disabled:opacity-50 shadow-lg"
              >
            ...
            <div className="min-h-[80px] lg:min-h-[120px] flex items-center justify-center">
              {isGenerating ? (
                ...
                <div className="text-xs md:text-sm text-slate-300 leading-relaxed font-medium whitespace-pre-wrap">
                  {takeaways}
                </div>
              ) : (
                <p className="text-[10px] md:text-xs text-slate-500 italic text-center leading-relaxed">Click the spark to generate key lesson insights using Shala AI.</p>
              )}
            </div>
          </div>
        </div>

        <div className="flex-1 bg-surface-dark border-l border-white/5 flex flex-col overflow-hidden text-left">
          <div className="p-8 border-b border-white/5 bg-slate-950/20">
             ...
             <p className="text-slate-500 text-[10px] font-bold mt-1 uppercase tracking-widest">5 VIDEO LESSONS • {course.duration}</p>
          </div>
            ...
            {lessons.map((lesson, index) => {
              ...
              return (
                <div key={lesson.id} onClick={() => !isLocked && setActiveLessonIndex(index)} className={`p-5 rounded-3xl flex items-center gap-5 cursor-pointer transition-all border-2 ${isActive ? 'bg-primary/10 border-primary/40 text-primary' : isLocked ? 'opacity-40 cursor-not-allowed border-transparent' : 'hover:bg-white/5 border-transparent text-slate-400'}`}>
                  <div className={`size-12 rounded-2xl flex items-center justify-center text-sm font-black shrink-0 transition-all ${isCompleted ? 'bg-green-500/20 text-green-500' : isActive ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white/5'}`}>
                  ...
                  <div className="flex-1 overflow-hidden">
                    <p className="text-sm font-bold truncate font-display">{lesson.title}</p>
