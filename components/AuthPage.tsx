...
const AuthPage: React.FC<AuthPageProps> = ({ course, onBack, onSuccess, initialMode = 'signup', isMaintenanceMode = false }) => {
  ...
  if (isAuthSuccess) {
    return (
      <div className="max-w-md mx-auto bg-white dark:bg-[#1f1f14] rounded-[32px] md:rounded-[40px] p-8 md:p-12 shadow-2xl animate-in zoom-in-95 duration-500 text-center border border-primary/20 flex flex-col items-center justify-center min-h-[350px] md:min-h-[400px]">
        <div className="size-20 md:size-24 rounded-full bg-primary flex items-center justify-center text-background-dark shadow-2xl shadow-primary/20 animate-bounce mb-6 md:mb-8">
          <span className="material-symbols-outlined text-4xl md:text-5xl font-black">check_circle</span>
        </div>
        <div className="space-y-2">
          <h2 className="text-2xl md:text-3xl font-black tracking-tight">Access Granted</h2>
          <p className="text-sm md:text-base text-slate-500 font-medium">Synchronizing your cloud profile with the Shala core...</p>
        </div>
  ...
  return (
    <div className="relative">
      <div className={`max-w-4xl mx-auto flex flex-col md:flex-row bg-white dark:bg-[#1f1f14] rounded-[32px] md:rounded-[40px] overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-700`}>
        <div className="md:w-5/12 bg-background-dark p-8 md:p-10 flex flex-col justify-between relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10">
            <div className={`size-10 md:size-12 ${isMaintenanceMode ? 'bg-red-500' : 'bg-primary'} rounded-xl md:rounded-2xl flex items-center justify-center text-background-dark mb-4 md:mb-6 shadow-xl shadow-primary/20`}>
            ...
            <div className={`p-4 rounded-2xl backdrop-blur-sm border text-left ${isMaintenanceMode ? 'bg-red-500/10 border-red-500/20' : 'bg-primary/10 border-primary/20'}`}>
          ...
          <div className="relative z-10 pt-8 md:pt-10 text-left hidden md:block">
            <div className="flex -space-x-3 mb-4">
              {[1, 2, 3, 4].map(i => (
                <img key={`auth-avatar-${i}`} className="size-8 rounded-full border-2 border-background-dark" src={`https://i.pravatar.cc/150?u=${i + 45}`} alt="" />
              ))}
              <div className="size-8 rounded-full bg-slate-800 border-2 border-background-dark flex items-center justify-center text-[10px] text-white font-bold tracking-tighter">+50k</div>
            </div>
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Secured by Supabase Backend</p>
          </div>
        </div>

        <div className="md:w-7/12 p-6 md:p-8 lg:p-14 h-full">
          <div className="flex justify-between items-center mb-8">
            <button onClick={onBack} className="text-slate-400 hover:text-primary transition-colors flex items-center gap-2">
          ...
          {(!isMaintenanceMode && (mode === 'login' || mode === 'signup')) && (
            <div className="flex p-1.5 bg-slate-100 dark:bg-slate-800 rounded-2xl mb-10 relative">
              <div 
                className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white dark:bg-[#1f1f14] rounded-xl shadow-sm transition-all duration-300 ease-out z-0 ${mode === 'signup' ? 'translate-x-full ml-1.5' : 'translate-x-0'}`}
              />
              <button 
                onClick={() => toggleMode('login')}
                className={`flex-1 py-3 text-sm font-black relative z-10 transition-colors duration-300 ${mode === 'login' ? 'text-primary' : 'text-slate-500 hover:text-slate-700'}`}
              >
                LOG IN
              </button>
              <button 
                onClick={() => toggleMode('signup')}
                className={`flex-1 py-3 text-sm font-black relative z-10 transition-colors duration-300 ${mode === 'signup' ? 'text-primary' : 'text-slate-500 hover:text-slate-700'}`}
              >
                SIGN UP
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 text-left">
            {mode === 'signup' && (
              <div className="space-y-1">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 dark:bg-[#393928] border-none rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-primary dark:text-white" 
            ...
            {(mode === 'login' || mode === 'signup' || mode === 'reset') && (
              <div className="space-y-1">
                ...
                <div className="relative group">
                  <input required type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-50 dark:bg-[#393928] border-none rounded-2xl px-5 py-4 pr-14 text-sm focus:ring-2 focus:ring-primary dark:text-white" placeholder="••••••••" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-primary p-2">
            ...
            <button disabled={isSubmitting} className={`w-full py-5 ${isMaintenanceMode ? 'bg-red-500' : 'bg-background-dark dark:bg-primary'} text-white dark:text-background-dark font-black rounded-3xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-70 mt-4 group uppercase tracking-widest text-sm`}>
              {isSubmitting ? (
