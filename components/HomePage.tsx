...
const HomePage: React.FC<HomePageProps> = ({ 
  ...
  return (
    <div className="animate-in fade-in duration-700">
      ...
      {showResultPopup && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-background-dark/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white dark:bg-[#1f1f14] w-full max-w-sm rounded-[40px] p-10 shadow-2xl animate-in zoom-in-95 duration-300 text-center space-y-6 border border-primary/20">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">True!</h3>
            ...
            <button 
              ...
              className="w-full py-4 bg-primary text-background-dark font-black rounded-2xl hover:scale-105 transition-all shadow-lg uppercase tracking-widest"
            >
              Continue Learning
            </button>
          </div>
      ...
      {showRequirementAlert && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white dark:bg-[#1f1f14] w-full max-w-sm rounded-[40px] p-10 shadow-2xl animate-in zoom-in-95 duration-300 text-center space-y-6 border border-primary/20">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-black">{showRequirementAlert.title}</h3>
              <p className="text-sm text-slate-500 leading-relaxed">{showRequirementAlert.msg}</p>
            </div>
            <button onClick={() => setShowRequirementAlert(null)} className="w-full py-4 bg-primary text-background-dark font-black rounded-2xl hover:scale-105 transition-all shadow-lg">GOT IT</button>
          </div>
        </div>
      )}

      {/* Hero Section */}
      <section className="relative px-5 sm:px-8 lg:px-12 py-16 md:py-32 overflow-hidden rounded-[32px] md:rounded-[60px] bg-background-dark text-white mb-12 md:mb-20 text-left">
        ...
        <div className="relative z-10 max-w-4xl">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] md:text-xs font-black uppercase tracking-widest mb-6 md:mb-8">
          ...
          <h1 className="text-4xl sm:text-6xl md:text-8xl font-black leading-[1] tracking-tighter mb-6 md:mb-8">
            ...
            <span className="text-primary">Define Careers.</span>
          </h1>
          <p className="text-base sm:text-lg md:text-2xl text-slate-400 max-w-2xl mb-10 md:mb-12 leading-relaxed font-bold">
          ...
          <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
            <button onClick={onBrowse} className="w-full sm:w-auto px-10 py-5 bg-primary text-background-dark font-black rounded-2xl shadow-2xl shadow-primary/20 hover:scale-[1.05] transition-all flex items-center justify-center gap-3 text-base md:text-lg">
              BROWSE CATALOG <span className="material-symbols-outlined">arrow_forward</span>
            </button>
            <button onClick={onLearnMore} className="w-full sm:w-auto px-10 py-5 bg-slate-800 text-white font-black rounded-2xl hover:bg-slate-700 transition-all text-base md:text-lg">LEARN MORE</button>
          </div>
        </div>

        <div className="mt-16 md:mt-24 grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 border-t border-white/10 pt-10 md:pt-12">
          <div className="text-left"><p className="text-3xl md:text-4xl font-black text-primary">500+</p><p className="text-[10px] md:text-xs text-slate-500 uppercase font-black tracking-widest">Expert Courses</p></div>
          <div className="text-left"><p className="text-3xl md:text-4xl font-black text-white">50k+</p><p className="text-[10px] md:text-xs text-slate-500 uppercase font-black tracking-widest">Active Students</p></div>
          <div className="text-left"><p className="text-3xl md:text-4xl font-black text-white">4.9/5</p><p className="text-[10px] md:text-xs text-slate-500 uppercase font-black tracking-widest">Average Rating</p></div>
          <div className="text-left"><p className="text-3xl md:text-4xl font-black text-white">92%</p><p className="text-[10px] md:text-xs text-slate-500 uppercase font-black tracking-widest">Career Advancement</p></div>
        </div>
      </section>

      {/* Daily AI Challenge Card */}
      <section className="mb-24 px-0">
        <div className="bg-gradient-to-br from-primary/20 via-primary/5 to-transparent border-2 border-primary/30 p-8 md:p-16 rounded-[48px] md:rounded-[64px] flex flex-col lg:flex-row items-center justify-between gap-12 text-left relative overflow-hidden">
           <div className="space-y-6 w-full lg:max-w-2xl relative z-10">
             ...
             <h2 className="text-3xl md:text-5xl font-black tracking-tighter">Test your industry knowledge.</h2>
             <p className="text-slate-700 dark:text-slate-300 text-lg md:text-2xl font-bold leading-tight py-2">
               "{challenge.question}"
             </p>
                ...
                {challenge.options.map((opt, i) => (
                  <button 
                    ...
                    className={`px-8 py-5 rounded-2xl text-xs md:text-sm font-black uppercase tracking-widest transition-all border-2 flex items-center justify-center gap-4 min-h-[64px] ${
                      selectedOption === null 
                        ? 'bg-background-dark text-white hover:bg-primary hover:text-background-dark border-transparent' 
                        : selectedOption === i 
           ...
           <div className="shrink-0 relative hidden lg:block">
              <div className="size-64 bg-primary/10 rounded-full flex items-center justify-center animate-pulse border-4 border-primary/20">
      ...
      <section className="mb-24 text-left">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-6 mb-10 md:mb-12">
          <div className="space-y-2">
            <h2 className="text-3xl md:text-5xl font-black tracking-tighter">Featured Programs</h2>
            <p className="text-sm md:text-lg text-slate-500 font-bold">Kickstart your journey with our popular specialized courses.</p>
          </div>
      ...
      <section className="mb-24 space-y-10 md:space-y-16">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
          <div className="space-y-1 md:space-y-2">
            <h2 className="text-3xl md:text-4xl font-black">Voices of Success</h2>
            <p className="text-xs md:text-sm text-slate-400 font-medium">Genuine feedback from Shala graduates.</p>
          </div>
          <button onClick={handleReviewClick} className="w-full md:w-auto flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 bg-white dark:bg-[#1f1f14] border-2 border-primary rounded-2xl md:rounded-3xl font-black hover:bg-primary hover:text-background-dark transition-all shadow-xl shadow-primary/10 text-xs md:text-sm">
            <span className="material-symbols-outlined text-lg md:text-xl">rate_review</span> ADD YOUR REVIEW
          </button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {userReviews.map((rev) => (
              <div key={rev.name} className={`relative p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-sm border text-left space-y-4 md:space-y-6 transition-transform hover:-translate-y-2 duration-300 ${ (rev as any).dark ? 'bg-background-dark text-white border-white/5 shadow-2xl' : 'bg-white dark:bg-[#1f1f14] border-slate-100 dark:border-slate-800' }`}>
                {rev.isNew && <div className="absolute -top-3 -right-3 bg-primary text-background-dark text-[8px] md:text-[10px] font-black px-3 md:px-4 py-1 md:py-1.5 rounded-full shadow-lg">NEW REVIEW</div>}
              ...
              <p className={`italic leading-relaxed text-sm md:text-base ${ (rev as any).dark ? 'text-slate-300' : 'text-slate-600 dark:text-slate-300'}`}> "{rev.text}" </p>
              <div className={`flex items-center gap-3 md:gap-4 border-t pt-4 md:pt-6 ${ (rev as any).dark ? 'border-white/5' : 'border-slate-50 dark:border-slate-800'}`}>
                <img src={rev.img} className="size-10 md:size-12 rounded-full ring-2 ring-primary/20" alt="" />
                <div><h4 className="font-black text-xs md:text-sm">{rev.name}</h4><p className={`text-[8px] md:text-[10px] uppercase font-bold tracking-tighter ${ (rev as any).dark ? 'text-primary' : 'text-slate-500'}`}> {rev.role} </p></div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Final CTA */}
      <section className="bg-primary rounded-[40px] md:rounded-[60px] p-8 md:p-24 text-center space-y-8 relative overflow-hidden shadow-2xl shadow-primary/20">
        <div className="absolute top-0 right-0 p-10 opacity-10 hidden md:block"><span className="material-symbols-outlined text-[200px] font-black">celebration</span></div>
        <h2 className="text-3xl md:text-7xl font-black text-background-dark leading-none tracking-tighter">Ready to start <br />your journey?</h2>
        <p className="text-background-dark/70 font-bold text-base md:text-xl max-w-xl mx-auto uppercase tracking-wide">Unlock 500+ premium industry certifications.</p>
        <button onClick={onBrowse} className="w-full sm:w-auto px-10 md:px-14 py-5 md:py-6 bg-background-dark text-white font-black rounded-2xl md:rounded-3xl hover:scale-110 active:scale-95 transition-all shadow-2xl text-base md:text-lg tracking-widest">JOIN SHALA TODAY</button>
      </section>
    </div>
  );
};
