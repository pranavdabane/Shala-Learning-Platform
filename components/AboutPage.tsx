...
const AboutPage: React.FC<AboutPageProps> = ({ onBack }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-24 pb-24">
      <button 
        onClick={onBack}
      ...
      <section className="text-center space-y-8 px-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/20 text-primary text-[10px] font-bold uppercase tracking-[0.2em]">
          The Shala Difference
        </div>
        <h1 className="text-5xl md:text-8xl font-black tracking-tighter leading-[0.9] font-display">
          Redefining <br />
          <span className="text-primary italic">Excellence.</span>
        </h1>
        <p className="text-lg md:text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed font-medium">
          Founded in 2018, Shala has grown from a boutique design academy to a global leader in AI-enhanced professional development.
        </p>
      </section>

      {/* Methodology Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 px-4">
        <div className="bg-white dark:bg-surface-dark p-12 rounded-[48px] shadow-xl border border-slate-100 dark:border-slate-800 space-y-8 hover:shadow-2xl transition-all duration-500">
          ...
          <h3 className="text-3xl font-black font-display">AI-Powered Pedagogy</h3>
        ...
        <div className="bg-slate-900 text-white p-12 rounded-[48px] shadow-2xl relative overflow-hidden space-y-8 hover:shadow-primary/10 transition-all duration-500">
          <div className="absolute top-0 right-0 p-12 opacity-10 pointer-events-none">
            <span className="material-symbols-outlined text-[160px] font-black">workspace_premium</span>
          </div>
          ...
          <h3 className="text-3xl font-black font-display">Industry Reputation</h3>
      ...
      <section className="space-y-16 px-4">
        <div className="text-center space-y-2">
          <h2 className="text-4xl font-black font-display">Elite Instructor Network</h2>
          <p className="text-slate-500 font-medium">Learn from the practitioners, not just academics.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="group flex flex-col items-center text-center p-10 bg-white dark:bg-surface-dark rounded-[40px] border border-slate-100 dark:border-slate-800 hover:border-primary/50 hover:shadow-2xl transition-all duration-500">
              <div className="relative mb-6">
                ...
                <div className="absolute -bottom-2 -right-2 size-8 bg-primary rounded-xl flex items-center justify-center text-white shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-500">
                  <span className="material-symbols-outlined text-sm">verified</span>
                </div>
              </div>
              <h4 className="font-bold text-xl mb-1">{ins.name}</h4>
      ...
      <section id="contact" className="space-y-12 px-4">
        <div className="text-center space-y-2">
          <h2 className="text-5xl font-black tracking-tighter font-display">Get in Touch</h2>
          <p className="text-slate-500 font-medium">We're here to support your professional evolution.</p>
        </div>
        <ContactForm userName={user.name} userEmail={user.email} />
      </section>

      {/* Global Impact */}
      <section className="bg-primary p-12 md:p-24 rounded-[60px] flex flex-col md:flex-row items-center justify-between gap-16 text-white mx-4 shadow-2xl shadow-primary/20 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-12 opacity-10 pointer-events-none">
          <span className="material-symbols-outlined text-[200px] font-black">public</span>
        </div>
