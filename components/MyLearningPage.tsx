...
const CertificateModal: React.FC<{ 
  ...
  return (
    <div className="fixed inset-0 z-[150] flex flex-col items-center justify-start p-4 md:p-12 bg-slate-950/95 backdrop-blur-2xl animate-in fade-in duration-500 overflow-y-auto no-scrollbar print:p-0 print:bg-white certificate-print-zone">
      ...
      {isSendingEmail && (
        <div className="fixed inset-0 z-[200] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6">
           <div className="bg-white dark:bg-surface-dark p-10 md:p-16 rounded-[48px] text-center space-y-8 max-w-md shadow-2xl border border-primary/20 animate-in zoom-in-95">
              ...
              <div className="space-y-3">
                 <h3 className="text-xl md:text-2xl font-black font-display">Emailing Certificate</h3>
                 <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm leading-relaxed font-medium min-h-[40px]">{sendStage}</p>
              </div>
           </div>
        </div>
      )}
      ...
      {isSent && (
        <div className="fixed top-12 left-1/2 -translate-x-1/2 z-[210] bg-green-500 text-white px-8 py-4 rounded-full shadow-2xl flex items-center gap-4 animate-in slide-in-from-top-12 duration-500 w-[90%] md:w-auto">
      ...
      <div className="absolute top-6 right-6 md:top-10 md:right-10 print:hidden">
        <button 
          onClick={onClose}
          className="size-12 md:size-16 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center transition-all shadow-xl border border-white/10"
        >
      ...
      <div className="flex flex-col items-center w-full max-w-[1200px] py-10 md:py-16 print:py-0">
                  ...
                  <div className="space-y-3 md:space-y-6">
                    <div className="flex items-center justify-center gap-3 md:gap-4 text-slate-900">
                      <div className="size-10 md:size-14 bg-primary rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg">
                        <span className="material-symbols-outlined font-black text-xl md:text-3xl text-white">school</span>
                      </div>
        ...
        <div className="mt-[-120px] sm:mt-[-60px] md:mt-16 flex flex-col items-center gap-6 md:gap-10 print:hidden w-full px-6 relative z-20">
          <div className="flex flex-col sm:flex-row items-center gap-6 md:gap-8 w-full sm:w-auto">
            <button 
              ...
              className="w-full sm:w-auto px-10 md:px-16 py-5 md:py-6 bg-primary text-white font-black rounded-3xl shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-4 border-2 border-white/20 group text-lg md:text-xl h-16 md:h-20 uppercase tracking-widest"
            >
            ...
            <button 
              onClick={onClose}
              className="w-full sm:w-auto px-10 md:px-12 py-5 md:py-6 bg-transparent hover:bg-white/5 text-slate-400 font-bold rounded-3xl transition-all text-lg md:text-xl h-16 md:h-20 uppercase tracking-widest"
            >
              CLOSE VIEWER
            </button>
          </div>
        ...
        <p className="mt-12 md:mt-20 text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-[0.5em] print:hidden text-center">
...
const MyLearningPage: React.FC<MyLearningPageProps> = ({ 
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-16 pb-24">
      {/* Loading Overlay */}
      {isPreparing && (
        <div className="fixed inset-0 z-[200] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-8">
           <div className="bg-white dark:bg-surface-dark p-12 rounded-[48px] text-center space-y-8 max-w-md shadow-2xl border border-primary/20 animate-in zoom-in-95">
              ...
              <div className="space-y-3">
                 <h3 className="text-2xl font-black font-display">Preparing Certificate</h3>
                 <p className="text-slate-500 dark:text-slate-400 text-sm leading-relaxed font-medium">Validating mastery data and secure credentials...</p>
              </div>
           </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 text-left">
        <div className="space-y-2">
          <h1 className="text-5xl font-black tracking-tighter leading-none font-display">My Learning</h1>
          <p className="text-slate-500 dark:text-slate-400 text-lg font-medium">Track your path to professional mastery.</p>
        </div>
        {enrolledCourses.length > 0 && (
          <div className="px-6 py-4 bg-primary/10 border-2 border-primary/20 text-primary rounded-2xl flex items-center gap-4 shadow-xl shadow-primary/5">
            <span className="material-symbols-outlined font-black">verified_user</span>
            <span className="text-xs font-bold uppercase tracking-[0.2em]">{enrolledCourses.length} Active Tracks</span>
          </div>
        )}
      </div>
      ...
      {enrolledCourses.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {enrolledCourses.map((course) => {
            ...
            return (
              <div 
                key={course.id}
                className="group bg-white dark:bg-surface-dark rounded-[48px] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 border border-slate-100 dark:border-slate-800 flex flex-col h-full"
              >
                <div className="relative h-64 overflow-hidden cursor-pointer shrink-0" onClick={() => setPlayingCourse(course)}>
                  ...
                  <div className="absolute inset-0 bg-slate-900/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <div className="size-20 rounded-full bg-primary flex items-center justify-center text-white shadow-2xl scale-75 group-hover:scale-100 transition-transform duration-500">
                        <span className="material-symbols-outlined text-4xl font-black">play_arrow</span>
                    </div>
                  </div>
                  
                  {isCompleted && (
                    <div className="absolute top-6 right-6 bg-green-500 text-white px-5 py-2 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] shadow-xl flex items-center gap-2">
                  ...
                  {isEligibleForCert && !isCompleted && (
                    <div className="absolute top-6 left-6 bg-primary text-white px-5 py-2 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] shadow-xl flex items-center gap-2 animate-in slide-in-from-left-6">
                ...
                <div className="p-8 md:p-12 space-y-8 flex-1 flex flex-col justify-between">
                  <div className="space-y-6">
                    <div className="text-left space-y-2">
                      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary block">{course.category}</span>
                      <h3 className="text-2xl font-black leading-tight group-hover:text-primary transition-colors font-display">{course.title}</h3>
                    </div>

                    <div className="space-y-4">
                      <div className="flex justify-between items-end">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Mastery Level</span>
                        <span className={`text-xl font-black font-display ${isEligibleForCert ? 'text-primary' : 'text-slate-400'}`}>{course.progress}%</span>
                      </div>
                      <div className="h-4 w-full bg-slate-100 dark:bg-slate-900 rounded-full overflow-hidden p-1">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ease-out ${isEligibleForCert ? 'bg-primary shadow-[0_0_15px_rgba(168,85,247,0.6)]' : 'bg-slate-400'}`}
                  ...
                  <div className="flex flex-col gap-4 pt-6">
                    <button 
                      ...
                      className="w-full py-5 rounded-2xl bg-slate-900 text-white dark:bg-slate-800 group-hover:bg-primary group-hover:text-white text-sm font-bold transition-all flex items-center justify-center gap-3 shadow-xl uppercase tracking-widest"
                    >
                    ...
                    {isEligibleForCert && (
                      <button 
                        ...
                        className="w-full py-5 rounded-2xl bg-primary/10 border-2 border-primary/20 text-primary text-sm font-bold transition-all flex items-center justify-center gap-3 hover:bg-primary hover:text-white shadow-xl uppercase tracking-widest"
                      >
        ...
        <div className="flex flex-col items-center justify-center py-32 px-10 bg-white dark:bg-surface-dark rounded-[60px] shadow-xl border border-slate-100 dark:border-slate-800 text-center space-y-12 max-w-4xl mx-auto border-dashed">
          <div className="size-48 rounded-[48px] bg-primary/5 flex items-center justify-center text-primary relative">
            <div className="absolute inset-0 rounded-[48px] border-4 border-dashed border-primary/20 animate-spin [animation-duration:20s]"></div>
            <span className="material-symbols-outlined text-8xl font-black">school</span>
          </div>
          
          <div className="space-y-4 max-w-xl">
            <h2 className="text-5xl font-black tracking-tighter font-display">Your curriculum is waiting.</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-8">
            <button 
              onClick={onBrowse}
              className="px-14 py-6 bg-primary text-white font-bold rounded-[32px] shadow-2xl shadow-primary/20 hover:scale-110 active:scale-95 transition-all flex items-center gap-4 uppercase tracking-widest text-sm"
            >
              BROWSE CATALOG
            ...
            <button 
              ...
              className="px-14 py-6 bg-slate-100 dark:bg-slate-900 font-bold rounded-[32px] hover:bg-slate-200 dark:hover:bg-slate-800 transition-all uppercase tracking-widest text-sm"
            >
              CAREER PATHS
            </button>
          </div>
