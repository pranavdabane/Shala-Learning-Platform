...
const CourseCard: React.FC<CourseCardProps> = ({ course, onClick, isWishlisted, onToggleWishlist }) => {
  return (
    <motion.div 
      ...
      className="group flex flex-col bg-card rounded-2xl overflow-hidden shadow-sm hover:shadow-[0_0_20px_rgba(230,255,0,0.25)] hover:ring-2 hover:ring-primary/40 transition-all duration-300 cursor-pointer relative border border-neon-border mx-auto w-full max-w-full"
    >
      <div className="relative w-full aspect-square sm:aspect-video bg-background-secondary overflow-hidden shrink-0">
        ...
        <div className="absolute top-2 right-2 sm:top-3 sm:right-3 bg-primary text-black text-xs sm:text-sm font-bold px-3 py-1 sm:px-4 sm:py-1.5 rounded-full shadow-[0_0_10px_rgba(230,255,0,0.3)]">
          ${course.price}
        </div>
      </div>
      <div className="p-4 sm:p-6 flex flex-col gap-2 sm:gap-3 flex-1 text-left min-w-0">
        ...
        <h3 className="text-base sm:text-xl font-black leading-tight line-clamp-2 group-hover:text-primary transition-colors break-words text-white">{course.title}</h3>
